(() => {
  ...
  function signIn() {
    if (leaving) return;
    leaving = true;
    location.replace(`/login?expired=1&next=${encodeURIComponent(location.pathname + location.search + location.hash)}`);
  }
  ...
  async function refresh() {
    try {
      ...
      if (response.ok) { const user = await response.json(); label.textContent = user.username; }
