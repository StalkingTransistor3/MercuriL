...
-- NULL is an unobserved channel, not a zero-depth measurement.
ALTER TABLE sensors ALTER COLUMN depth_m DROP NOT NULL;
ALTER TABLE sensors ALTER COLUMN battery_pct DROP NOT NULL;
ALTER TABLE sensors ALTER COLUMN last_seen DROP NOT NULL;
ALTER TABLE sensor_readings ALTER COLUMN depth_m DROP NOT NULL;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS observed_at timestamptz;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS state_observed_at timestamptz;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS last_contact timestamptz;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS telemetry_src text;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS report_interval_s integer;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS class_derived boolean NOT NULL DEFAULT false;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS is_simulated boolean NOT NULL DEFAULT false;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS deployment text NOT NULL DEFAULT 'bench'
  CHECK (deployment IN ('bench','installed'));
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS location_note text;
ALTER TABLE sensors ADD COLUMN IF NOT EXISTS rock7_imei text;
CREATE UNIQUE INDEX IF NOT EXISTS sensors_rock7_imei_idx ON sensors(rock7_imei) WHERE rock7_imei IS NOT NULL;

CREATE TABLE IF NOT EXISTS sensor_closures (
  id bigserial PRIMARY KEY,
  sensor_id integer NOT NULL REFERENCES sensors(id) ON DELETE CASCADE,
  ...
  class_derived boolean NOT NULL,
  depth_m double precision,
  velocity_ms double precision,
  dv_product double precision
);
CREATE UNIQUE INDEX IF NOT EXISTS sensor_closures_active_idx
  ON sensor_closures(sensor_id) WHERE reopened_at IS NULL;
ALTER TABLE sensor_closures ADD COLUMN IF NOT EXISTS assessment jsonb;
`;
...
async function initDb(db = getPool()) {
