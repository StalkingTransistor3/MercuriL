...
async function osrmRoute(from, to) {
  ...
  const resp = await throttled(() => fetch(url, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(15_000) }));
...
function currentEvidence(result, sensors) {
  const byId = new Map(sensors.map((s) => [s.id, s]));
  const copy = { ...result };
  for (const key of ['hazards', 'avoided']) {
    if (copy[key]) copy[key] = copy[key].map((s) => byId.get(s.id)).filter(Boolean);
  }
  return copy;
}
...
async function route(from, to, mode, floodedSensors) {
  ...
  if (cached && Date.now() - cached.at < (cached.result.avoidanceUnavailable ? 60_000 : 300_000)) {
    return currentEvidence(cached.result, floodedSensors);
  }
  ...
  if (mode === 'mercuril' && hazards.length) {
    ...
    try {
      ...
      // An HTTP 200 is not proof the engine honoured the exclusion polygons.
      if (hazardsOnRoute(detour.coords, floodedSensors).length) detour = null;
    } catch (e) {
